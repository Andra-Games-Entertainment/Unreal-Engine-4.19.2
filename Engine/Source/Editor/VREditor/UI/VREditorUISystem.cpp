...
namespace VREd
{
	...
	static FAutoConsoleVariable AllowAssetEditor( TEXT( "VREd.AllowAssetEditor" ), 0, TEXT( "Whether it is possible to open the asset editor" ) );
	...
	{
		{
			...
			if( GIsDemoMode )
			{
				...
				Config.bUseSourcesView = false;
				Config.ThumbnailScale = 0.65f;
				...
				Config.SelectedCollectionName.Name = TEXT( "DemoAssets" );
		...
		{
			if (VREd::AllowAssetEditor->GetInt() == 1)
			{
...
void UVREditorUISystem::TryToSpawnRadialMenu( UVREditorInteractor* Interactor, const bool bForceRefresh, const bool bPlaySound /*= true*/ )
{
	if( Interactor != nullptr )
	{
		...
		if( MotionControllerInteractor != nullptr )
		{
			...
			if( GetOwner().GetWorldInteraction().ShouldTemporarilyHideLasers() ||
				GetOwner().bIsCalibrating )
			{
				bNeedsSpawn = false;
			}
