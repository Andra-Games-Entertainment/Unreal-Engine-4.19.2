...
void FMorphTargetVertexInfoBuffers::InitRHI()
{
...

	{
		...
		VertexIndicesVB = RHICreateStructuredBuffer(4, VertexIndices.GetAllocatedSize(), BUF_Static | BUF_ShaderResource, CreateInfo);
		void* VertexIndicesVBData = RHILockStructuredBuffer(VertexIndicesVB, 0, VertexIndices.GetAllocatedSize(), RLM_WriteOnly);
		FMemory::Memcpy(VertexIndicesVBData, VertexIndices.GetData(), VertexIndices.GetAllocatedSize());
		RHIUnlockStructuredBuffer(VertexIndicesVB);
		VertexIndicesSRV = RHICreateShaderResourceView(VertexIndicesVB);
	}
	{
		FRHIResourceCreateInfo CreateInfo;
