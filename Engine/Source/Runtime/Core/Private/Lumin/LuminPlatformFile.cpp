...
*/
// @todo nova - Consider if we need managed handles or not.
...
FString FLuminPlatformFile::ConvertToLuminPath(const FString& Filename, bool bForWrite) const
{
	...
	else
	{
		...
		{
//			Result = Filename;
			// @todo Lumin: Don't know why the Nova code didn't have this
...
#if 0
	FPlatformMisc::LowLevelOutputDebugStringf(TEXT("LOG_NOVA_PATH Write = %d Input = %s Output = %s"), bForWrite, *Filename, *Result.ToLower());
#endif
