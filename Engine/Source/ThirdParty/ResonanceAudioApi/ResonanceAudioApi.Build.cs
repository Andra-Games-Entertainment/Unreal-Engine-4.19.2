...
public class ResonanceAudioApi : ModuleRules
{
	public ResonanceAudioApi(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (Target.Platform == UnrealTargetPlatform.Android)
		{
			...
			AdditionalPropertiesForReceipt.Add(new ReceiptProperty("AndroidPlugin", Path.Combine(PluginPath, "ResonanceAudio_APL.xml")));
		...
		{
			string PluginPath = Utils.MakePathRelativeTo(ModuleDirectory, Target.RelativeEnginePath);
			AdditionalPropertiesForReceipt.Add(new ReceiptProperty("LuminPlugin", Path.Combine(PluginPath, "Lumin/ResonanceAudio_LPL.xml")));
