...
#pragma warning restore CS1591
	...
	class LuminPlatform : AndroidPlatform
    {
		...
		public override bool HasSpecificDefaultBuildConfig(UnrealTargetPlatform Platform, DirectoryReference ProjectPath)
		{
			...
			if (!DoProjectSettingsMatchDefault(UnrealTargetPlatform.Android, ProjectPath, "/Script/LuminRuntimeSettings.LuminRuntimeSettings", BoolKeys, null, null))
            {
                return false;
            }
            ...
            if ((Target.Platform == UnrealTargetPlatform.Win32) || (Target.Platform == UnrealTargetPlatform.Win64) || (Target.Platform == UnrealTargetPlatform.Mac) || (Target.Platform == UnrealTargetPlatform.Linux))
            {
                ...
                if (!Target.bBuildRequiresCookedData)
                {
                    if (ModuleName == "Engine")
                    {
                        ...
                        {
                            Rules.PlatformSpecificDynamicallyLoadedModuleNames.Add("LuminTargetPlatform");
                        }
                    }
		...
		public override void SetUpSpecificEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment CompileEnvironment, LinkEnvironment LinkEnvironment)
        {
            ...
            CompileEnvironment.IncludePaths.SystemIncludePaths.Add("$(MLSDK)/lumin/stl/gnu-libstdc++/include");
            CompileEnvironment.IncludePaths.SystemIncludePaths.Add("$(MLSDK)/lumin/stl/gnu-libstdc++/include/aarch64-linux-android");
            CompileEnvironment.IncludePaths.SystemIncludePaths.Add("$(MLSDK)/include");

            LinkEnvironment.LibraryPaths.Add("$(MLSDK)/lib/lumin");
            LinkEnvironment.LibraryPaths.Add("$(MLSDK)/lumin/stl/gnu-libstdc++/lib");
            ...
            {
                LinkEnvironment.LibraryPaths.Add(TegraDebuggerDir + "/target/android-kk-egl-t124-a32");
                LinkEnvironment.LibraryPaths.Add(TegraDebuggerDir + "/target/android-L-egl-t132-a64");
        ...
        public static bool UseTegraGraphicsDebugger(FileReference ProjectFile)
        {
            ...
            Ini.GetBool(
                "/Script/NovaRuntimeSettings.NovaRuntimeSettings",
                "bBuildWithNvTegraGfxDebugger",
                out bBuild);
	...
	class LuminPlatformSDK : AndroidPlatformSDK
	{
		...
		/// </summary>
		static string ExpectedSDKVersion = "0.11";   // now unified for all the architectures
