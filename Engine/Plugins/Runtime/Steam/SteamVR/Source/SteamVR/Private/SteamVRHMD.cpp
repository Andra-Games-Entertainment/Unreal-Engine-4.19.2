...
// Useful for repeatable benchmarking
static TAutoConsoleVariable<int32> CDisablePoseUpdates(TEXT("vr.SteamVR.DisablePoseUpdates"), 0, TEXT("Disable pose updating. Useful for benchmarking."));
static TAutoConsoleVariable<int32> CLeftEyeOverscan(TEXT("vr.SteamVR.LeftEyeOverscan"), 0, TEXT("Whether to render the left eye at a larger widescreen resolution, then crop a column from the center for the HMD.  This is a presentation mode feature."));
static TAutoConsoleVariable<float> CLeftEyeOverscanAspect(TEXT("vr.SteamVR.LeftEyeOverscanAspect"), 16.0f / 9.0f, TEXT("Aspect ratio when vr.SteamVR.LeftEyeOverscan is enabled"));
static TAutoConsoleVariable<float> CRightEyeResolutionScaling(TEXT("vr.SteamVR.RightEyeResolutionScaling"), 1.0f, TEXT("Scales the resolution of the right eye only if vr.SteamVR.LeftEyeOverscan is active."));
...
void FSteamVRHMD::GetCurrentPose(FQuat& CurrentOrientation, FVector& CurrentPosition, uint32 DeviceId, EPoseRefreshMode RefreshMode/* = EPoseRefreshMode::None*/, float ForceRefreshWorldToMetersScale /* = 0.0f */ )
{
	...
	if (RefreshMode != EPoseRefreshMode::None)
	{
		...
		if (CDisablePoseUpdates.GetValueOnAnyThread() == 1)
		{
...
bool FSteamVRHMD::AllocateRenderTargetTexture(uint32 Index, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, uint32 InTexFlags, uint32 InTargetableTextureFlags, FTexture2DRHIRef& OutTargetableTexture, FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
{
	...
	pBridge->TextureSet = new FRHITextureSet2D(SwapChainLength, PF_B8G8R8A8, SizeX, SizeY, 1, NumSamples, InTexFlags, FClearValueBinding(FLinearColor::Transparent));
	
	for (uint32 SwapChainIter = 0; SwapChainIter < SwapChainLength; ++SwapChainIter)
	{
		IOSurfaceRef Surface = pBridge->GetSurface(SizeX, SizeY);
		check(Surface != nil);
		...
		static_cast<FRHITextureSet2D*>(pBridge->TextureSet.GetReference())->AddTexture(TargetableTexture, SwapChainIter);
	}
	
	OutTargetableTexture = OutShaderResourceTexture = pBridge->TextureSet;
...
ESteamVRTrackedDeviceType FSteamVRHMD::GetTrackedDeviceType(uint32 DeviceId) const
{
	...
	case vr::TrackedDeviceClass_GenericTracker: // @valve TODO: other was removed, we have GenericTracker now, percolate into ESteamVRTrackedDeviceType
...
void FSteamVRHMD::ResetOrientation(float Yaw)
{
	if( GIsDemoMode )
	{
		check( 0 );	// Hack for demo, we never want this function to be called as it interferes with our calibration process
	}
...
void FSteamVRHMD::AdjustViewRect(EStereoscopicPass StereoPass, int32& X, int32& Y, uint32& SizeX, uint32& SizeY) const
{
	...
	if (CLeftEyeOverscan->GetInt() != 0)
	{
		...
		{	
			SizeX = static_cast<uint32>(static_cast<float>(SizeX) * GameOverscanState.ModifiedEyeAspectRatio);
		}
		else
		{
			X = static_cast<int32>(static_cast<float>(X) * GameOverscanState.ModifiedEyeAspectRatio);
			SizeX *= GameOverscanState.RightEyeScale;
			SizeY *= GameOverscanState.RightEyeScale;
		}
	}
...
FMatrix FSteamVRHMD::GetStereoProjectionMatrix(const enum EStereoscopicPass StereoPassType, const float FOV) const
{
	...
	if (StereoPassType == EStereoscopicPass::eSSP_LEFT_EYE && CLeftEyeOverscan->GetInt() != 0)
	{
		Bottom /= GameOverscanState.ModifiedEyeAspectRatio;
		Top /= GameOverscanState.ModifiedEyeAspectRatio;
	}
...
void FSteamVRHMD::SetupView(FSceneViewFamily& InViewFamily, FSceneView& InView)
{
	...
	const bool bIsOverscan = (CLeftEyeOverscan->GetInt() != 0);
	if (!bIsOverscan)
	{
		if (InView.StereoPass != eSSP_FULL)
		{
			const uint32 ViewIndex = (InView.StereoPass == eSSP_LEFT_EYE ? 0 : 1);

			FScopeLock Lock(&FrameSettingsLock);
			InView.ViewRect = FrameSettings.EyeViewports[ViewIndex];
			InView.ResolutionOverrideRect = FrameSettings.MaxViewports[ViewIndex];
		}
	}
}
...
void FSteamVRHMD::CalculateRenderTargetSize(const class FViewport& Viewport, uint32& InOutSizeX, uint32& InOutSizeY)
{
	...
	else
	{
		...
		GameOverscanState.RightEyeScale = FMath::Clamp(CRightEyeResolutionScaling->GetFloat(), 0.25f, 1.0f);
		GameOverscanState.OriginalEyeAspectRatio = (static_cast<float>(InOutSizeX) * 0.5f) / static_cast<float>(InOutSizeY);
		GameOverscanState.ModifiedEyeAspectRatio = CLeftEyeOverscanAspect->GetFloat() / GameOverscanState.OriginalEyeAspectRatio;
		GameOverscanState.LeftEyeWidth = static_cast<uint32>(static_cast<float>(InOutSizeX) * 0.5f * GameOverscanState.ModifiedEyeAspectRatio);
		GameOverscanState.FamilyWidth = GameOverscanState.LeftEyeWidth;
		GameOverscanState.FamilyWidth += static_cast<uint32>(static_cast<float>((InOutSizeX) * 0.5f));
		InOutSizeX = GameOverscanState.FamilyWidth;

		// Update render thread copy
		{
			FScopeLock FrameLock(&OverscanMutex);
			RenderOverscanState = GameOverscanState;
		}
	}
