...
#elif PLATFORM_MAC
	TRefCountPtr<MetalBridge>	pBridge;
#elif PLATFORM_LINUX
	#if STEAMVR_USE_VULKAN_RHI
...
private:
	struct FOverscanState
	{
		float OriginalEyeAspectRatio;
		float ModifiedEyeAspectRatio;
		float RightEyeScale;
		uint32 LeftEyeWidth;
		uint32 FamilyWidth;

		FOverscanState()
			: OriginalEyeAspectRatio(1.0f)
			, ModifiedEyeAspectRatio(1.0f)
			, RightEyeScale(1.0f)
			, LeftEyeWidth(0)
			, FamilyWidth(0)
		{}
	...
	FOverscanState GameOverscanState;
	FOverscanState RenderOverscanState;
	mutable FCriticalSection OverscanMutex;
};
