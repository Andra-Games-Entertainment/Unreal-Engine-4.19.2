...
FMagicLeapMediaPlayer::~FMagicLeapMediaPlayer()
{
	...
	if (MLHandleIsValid(MediaPlayerHandle))
	{
		if (GSupportsImageExternal && !FLuminPlatformMisc::ShouldUseVulkan())
		{
			...
			FReleaseVideoResourcesParams ReleaseVideoResourcesParams = { TextureData, PlayerGuid, MediaPlayerHandle };
			...
			ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(MagicLeapMediaPlayerWriteVideoSample,
				FReleaseVideoResourcesParams, Params, ReleaseVideoResourcesParams,
				{
					...
					{
						MLMediaPlayerReleaseBuffer(Params.MediaPlayerHandle, Params.TextureData->PreviousNativeBuffer);
					}
...
bool FMagicLeapMediaPlayer::SetRate(float Rate)
{
	...
	{
		bResult = MLMediaPlayerStart(MediaPlayerHandle);
		if (bResult)
		{
...
void FMagicLeapMediaPlayer::TickFetch(FTimespan DeltaTime, FTimespan Timecode)
{
	...
	if (GSupportsImageExternal)
	{
		...
		FWriteVideoSampleParams WriteVideoSampleParams = { TextureData, PlayerGuid, MediaPlayerHandle };
		...
		ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(MagicLeapMediaPlayerWriteVideoSample,
			FWriteVideoSampleParams, Params, WriteVideoSampleParams,
			{
				...
				if (!(MLMediaPlayerPollingStateFlag_IsBufferAvailable & MLMediaPlayerPollStates(Params.MediaPlayerHandle, MLMediaPlayerPollingStateFlag_IsBufferAvailable)))
				{
					return;
				}
				...
				bool bResult = MLMediaPlayerAcquireNextAvailableBuffer(Params.MediaPlayerHandle, &nativeBuffer);

				if (bResult)
				{
					int32 CurrentFramePosition = 0;
					MLMediaPlayerGetCurrentPosition(Params.MediaPlayerHandle, &CurrentFramePosition);
				}
				else
				{
					return;
				...
				{
					MLMediaPlayerReleaseBuffer(Params.MediaPlayerHandle, TextureDataPtr->PreviousNativeBuffer);
				}
	...
	else
	{
		...
		WriteVideoSampleParams = { TextureData, MediaPlayerHandle, Samples, VideoSample, &CriticalSection, MediaWorker };
		...
		ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(MagicLeapMediaPlayerWriteVideoSample,
			FWriteVideoSampleParams, Params, WriteVideoSampleParams,
			{
				...
				else
				{
					...
					int32 currentPositionMilSec = 0;
					bResult = MLMediaPlayerGetCurrentPosition(Params.MediaPlayerHandle, &currentPositionMilSec);
					...
					MLMediaPlayerReleaseBuffer(Params.MediaPlayerHandle, nativeBuffer);
