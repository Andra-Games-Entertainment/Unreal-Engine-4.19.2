...
bool FMagicLeapHMD::IsHMDConnected()
{
	...
	return AppFramework.IsInitialized() && bIsVDZIEnabled && MLVirtualDeviceIsServerRunning();
#else
	return false;
#endif
}
...
{
	bStereoEnabled = (bHmdEnabled) ? bStereo : false;
...
bool FMagicLeapHMD::SetIgnoreInput(bool Ignore)
{
	...
	if (GetGameViewportClient() && GIsEditor)
	{
		bool result = GetGameViewportClient()->IgnoreInput();
		GetGameViewportClient()->SetIgnoreInput(Ignore);
		if (DisableInputForBeginPlay && !Ignore)
		{
			...
			GetGameViewportClient()->SetCaptureMouseOnClick(EMouseCaptureMode::NoCapture);
			GetGameViewportClient()->SetMouseLockMode(EMouseLockMode::DoNotLock);
			GetGameViewportClient()->SetHideCursorDuringCapture(false);
		}
		return result;
	}
#endif
...
void FMagicLeapHMD::UpdateNearClippingPlane()
{
	...
	if (frame != nullptr)
	{
		GNearClippingPlane = (frame->NearClippingPlane > GNearClippingPlane) ? frame->NearClippingPlane : GNearClippingPlane;
	}
}
	...
	if (!bDeviceInitialized)
	{
		...
		if (IsVulkanPlatform(GMaxRHIShaderPlatform))
		{
			...
			ExecuteOnRHIThread_DoNotWait([this, gfx_opts]()
			{
				...
				bool bResult = MLGraphicsGetRenderTargets(GraphicsClient, &render_targets_info, &OutStatus);
